...
if __name__ == '__main__':
	...
	data_subcat = pd.read_sql_query("""
		SELECT * 
		FROM sub_cat
	""", conn)

	print(data_subcat.shape)

	nrow = data_subcat.shape[0]
	conn.close()
	...
	lowest_categories_id = []
	for key, value in relation.items():
		if len(value) == 0:
			lowest_categories_id.append(key)


	print('Number of lowest categories:', len(lowest_categories_id))

	# got a list of 2133 categories
	# strategy is:
	# break it down to 2 list 
	# output each into a file
	# run 2 scripts to scrape each of them

	list1 = lowest_categories_id[:1000]
	list2 = lowest_categories_id[1000:]
	
	np.save('list1.npy', list1)
	np.save('list2.npy', list2)
	data_subcat.to_csv('data_subcat.csv', index=False, header=True)
	exit(0)
	...
	page_num = 1
	while True:
		tiki_page_url = tiki_webpage + str(page_num)	
		print('Scaping page:', tiki_page_url)
		sleep(randint(1,4))
		soup =	get_tiki_data(tiki_page_url) 
		if soup == None:
